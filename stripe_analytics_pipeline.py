...
def load_data(
    ...
    pipeline = dlt.pipeline(
        pipeline_name="stripe_analytics_tst2",
        destination='bigquery',
        dataset_name="stripe_tst_2",
        progress="log"
    )
    source = stripe_source(
        endpoints=endpoints, start_date=start_date, end_date=end_date
    )
    source.Event.apply_hints(primary_key="id")
    source.Invoice.apply_hints(primary_key="id")
    source.Subscription.apply_hints(primary_key="id")
    source.BalanceTransaction.apply_hints(primary_key="id")
