...
{% set active_ids_exist = adapter.get_relation(
        ...
        schema=this.schema,
        identifier="stripe_analytics_tst1_dlt_processed_load_ids" ) %}
...
AND load_id NOT IN (SELECT load_id FROM {{ source('transformed_data', 'stripe_analytics_tst1_dlt_processed_load_ids') }})
{% endif %}